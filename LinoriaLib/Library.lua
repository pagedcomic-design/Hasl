...
local BaseURL = "https://rbreset.top/LinoriaLib/"
...
AccentColor = Color3.fromRGB(192, 118, 227);
...
return (loadstring(
game:HttpGet("https://rbreset.top/lucide/source.lua")
) :: () -> IconModule)()
end)
...
function Library:GetTextBounds(Text, Font, Size, Resolution)
	...
	if CleanText:match("[^\32-\126]") then
		Width = Width + math.ceil(Width * 0.1) + 4
	end

	return Width, Bounds.Y
end
...
function BaseGroupboxFuncs:AddListbox(Idx, Info)
	assert(Info.Values, string.format("AddListbox (IDX: %s): Missing listbox value list.", tostring(Idx)))
	...
	Callback = Info.Callback or function(Value) end;
	Changed = Info.Changed or function(Value) end;
	MaxVisibleItems = Info.MaxVisibleItems or 8;
	}
	...
	BackgroundColor3 = Color3.new(0, 0, 0);
	BorderColor3 = Color3.new(0, 0, 0);
	Size = UDim2.new(1, -4, 0, (Listbox.MaxVisibleItems * 20) + 2);
	Visible = Listbox.Visible;
	ZIndex = 5;
	Parent = Container;
	})

	Library:AddToRegistry(ListOuter, { BorderColor3 = "Black" })
	...
	function Listbox:BuildList()
		...
		for _, Value in next, Listbox.Values do
			...
			local IsDisabled = table.find(Listbox.DisabledValues, StringValue)
			Count = Count + 1
			...
			local Selected = (Listbox.Multi and Listbox.Value[Value]) or (Listbox.Value == Value)
			ButtonLabel.TextColor3 = Selected and Library.AccentColor or (IsDisabled and Library.DisabledAccentColor or Library.FontColor)

			Library:AddToRegistry(ButtonLabel, {
			TextColor3 = Selected and "AccentColor" or (IsDisabled and "DisabledAccentColor" or "FontColor")
			})

			Button.MouseButton1Click:Connect(function()
			if Listbox.Disabled or IsDisabled then return end
			if Listbox.Multi then
				Listbox.Value[Value] = not Listbox.Value[Value]
			else
				Listbox.Value = Value
			end
...
function Listbox:SetVisible(Visibility)
	Listbox.Visible = Visibility
	ListOuter.Visible = Visibility
	Groupbox:Resize()
end

Listbox:BuildList()
Groupbox:AddBlank(5, Listbox.Visible)
Groupbox:Resize()

table.insert(Groupbox.Elements, Listbox)
Options[Idx] = Listbox
return Listbox
end
...
function Data:Resize()
	...
	pcall(NotifyOuter.TweenSize, NotifyOuter, UDim2.new(0, XSize * DPIScale + 20 + ExtraWidth, 0, YSize), "Out", "Quad", 0.4, true)
end
